...
class Compiler:
    def getFuncList(self, index: int, ast_list: List[List[Node]], func_list: List[str]) -> List[str]:
        ...
        if type(ast_list[index][0]) == FunctionNode:
            empty_var_list = []
            func_args_list, var_list = self.visit(ast_list[index][0], empty_var_list, func_list, index)
    ...
    def visit(self, node: Node, var_list: List[Tuple[str, int]], func_list: List[str], func_num: int) -> Tuple[List[str], List[Tuple[str, int]]]:
        method_name = f'visit_{type(node).__name__}'
        method = getattr(self, method_name)
        return method(node, var_list, func_list, func_num)

    def visit_FunctionNode(self, node: FunctionNode, var_list: List[Tuple[str, int]], func_list: List[str], func_num: int) -> Tuple[List[str], List[Tuple[str, int]]]:
        ...
        new_func_list = self.goThroughBody(index, node, vars_list, func_args_list, func_num)

        func_args_end = new_func_list+[""]
        return func_args_end, var_list

    def visit_VarNode(self, node: VarNode, vars_list: List[Tuple[str, int]], func_list: List[str], func_num: int) -> Tuple[List[str], List[Tuple[str, int]]]:
    ...
    def visit_VarAssignNode(self, node: VarAssignNode, vars_list: List[Tuple[str, int]], func_list: List[str], func_num: int) -> Tuple[List[str], List[Tuple[str, int]]]:
    ...
    def visit_IfNode(self, node: IfNode, vars_list: List[Tuple[str, int]], func_list: List[str], func_num: int) -> Tuple[List[str], List[Tuple[str, int]]]:
        ...
        if type(node.cases[0][1].right_node) == NumberNode:
            ...
            func_list, vars_list = self.visit(node.cases[0][0], vars_list, empty_func_list, func_num)
        ...
        print("right: ", node.cases[0][1].right_node.token[1])
        return func_list, vars_list

    def visit_WhileNode(self, node: WhileNode, vars_list: List[Tuple[str, int]], func_list: List[str], func_num: int) -> Tuple[List[str], List[Tuple[str, int]]]:
        ...
        while_func_list, vars_list = self.visit(node.body_node, vars_list, branch2_func_list, func_num)
    ...
    def visit_ReturnNode(self, node: ReturnNode, vars_list: List[Tuple[str, int]], func_list: List[str], func_num: int) -> Tuple[List[str], List[Tuple[str, int]]]:
        print(type(node.return_value))
    ...
    def goThroughBody(self, index: int, node: FunctionNode, vars_list: List[Tuple[str, int]], func_list: List[str], func_num: int) -> List[str]:
        # Check for ReturnNode
        if type(node.body[index][0]) == ReturnNode:
            return_func_list, vars_list = self.visit(node.body[index][0], vars_list, func_list, func_num)
            return return_func_list

        new_func_list, vars_list = self.visit(node.body[index][0], vars_list, func_list, func_num)
        return self.goThroughBody(index+1, node, vars_list, new_func_list, func_num)

    def getIfBranches(self, branches: List[str], node: IfNode, vars_list: List[Tuple[str, int]], func_num: int) -> List[str]:
        print(node.cases[0][0])
        return branches
